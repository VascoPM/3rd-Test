...
def model(window_length, latent_layer_size, activation_fn):
    ...
    layer_e1 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(inputs) 
    
    layer_e2 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(layer_e1)  
    
    layer_e3 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(layer_e2)    
    
    layer_e4 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(layer_e3)
    #Latent Space (no activation)
    encoded = Dense(latent_layer_size)(layer_e4)
       
    layer_d1 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(encoded)
    
    layer_d2 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(layer_d1)
    
    layer_d3 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(layer_d2)
    
    layer_d4 = Dense(300, activation = ann_train.get_activation_fn(activation_fn),
                     kernel_initializer = ann_train.get_initialization(activation_fn))(layer_d3) 
